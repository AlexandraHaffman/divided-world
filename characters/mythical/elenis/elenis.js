/* ══════════════════════════════════════════════════════════════
   ...
   · ПРЕВРАЩЕНИЕ — 29 стадий ALCHIMIA перекрашивают секцию по мере
                   прокрутки ленты: нигредо → альбедо → рубедо.
   · СОРОК СЕМЬ  — скраббер по годам 2014–2061. Декорации меняются,
...
/* 29 стадий Великого Превращения — алхимический символ, музейный комментарий
   и фаза цикла. Фаза перекрашивает секцию, пока лента прокручивается. */
const ALCHIMIA_GLOSS = {
  "ALC-001": { n: "I",     sym: "◎", ph: "materia",  note: "Материя без формы — начало Превращения." },
  ...
  "ALC-012": { n: "XII",   sym: "☉", ph: "metalla",  note: "Солнце в золоте — цель Превращения." },
  ...
  "ALC-029": { n: "XXIX",  sym: "✺", ph: "lapis",    note: "Квинтэссенция: всё Превращение, собранное в одном кадре." }
};
...
const PHASE_LABEL = {
  ...
  /* не «ПРЕВРАЩЕНИЯ»: так теперь называется вся коллекция, и прибор
     показывал бы фазу тем же словом, что и цикл целиком. Стадии здесь —
     лёд, расплав, дым, обсидиан, то есть агрегатные состояния. */
  mutatio:  "СОСТОЯНИЯ",
  lapis:    "КАМЕНЬ"
};
...
function buildReading() {
  ...
  if (stages >= 20) {
    lines.push(`Ты досмотрел Превращение почти до конца — <b>${stages}</b> ${plural(stages, "стадия", "стадии", "стадий")} из двадцати девяти. Сорок шесть лет съёмок, и ни одного объяснения зачем. Объяснение тебе и не понадобилось.`);
  } else if (stages >= 8) {
    lines.push(`Ты прошёл <b>${stages}</b> ${plural(stages, "стадию", "стадии", "стадий")} Превращения и остановился. Все останавливаются. Она — нет.`);
  }
