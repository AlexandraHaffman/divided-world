...
#    cuts       — вместо by_country/by_state: границы не наследуются ниоткуда,
#                 а проводятся вручную линиями разреза. Куски собирает
#                 shapely, каждый достаётся региону, чья столица внутри.
#                 Так нарезан Австралийский протекторат.
#    shapes     — необязательное: регион, у которого нет бывшей границы.
#                 Очертания задаются вручную, [широта, долгота], и
#                 вырезаются из всех соседних образцов. Так сделан Эриду:
#                 месопотамские болота не совпадают ни с одной мухафазой.
...
FACTIONS["tenebrion"] = {
  ...
  "regions": [
    ...
    ("scandinavia", "Скандинавский экзархат", "Копенгаген",            55.68,    12.57),
  ],
  "by_country": {
    "Austria": "vienna",
...
FACTIONS["forge"] = {
  "title": "Forge",
  "regions": [
    ("nanjing",   "Провинция Нижней Янцзы",     "Нанкин",     32.06,   118.80),
    ("fuzhou",    "Юго-Восточная провинция",    "Фучжоу",     26.07,   119.30),
    ("guangzhou", "Южная провинция",            "Гуанчжоу",   23.13,   113.26),
    ("wuhan",     "Центральная провинция",      "Ухань",      30.59,   114.31),
    ("xian",      "Западная провинция",         "Сиань",      34.34,   108.94),
    ("tianjin",   "Бохайская провинция",        "Тяньцзинь",  39.13,   117.20),
    ("shenyang",  "Северо-Восточная провинция", "Шэньян",     41.80,   123.43),
    ("taipei",    "Тайваньская провинция",      "Тайбэй",     25.03,   121.57),
  ],
  "by_country": {
    "Taiwan": "taipei",

    # Дельта Жемчужной реки вместе с бывшими особыми районами.
    "Hong Kong S.A.R.": "guangzhou", "Macao S.A.R": "guangzhou",

    # Корейского полуострова в списке провинций нет, а внутри территории
    # он лежит целиком. Отдан Северо-Восточной провинции — единственной
    # соседней с ним по суше.
    "North Korea": "shenyang", "South Korea": "shenyang",
  },
  # Китай разрезан по своим бывшим провинциям. Те, что заходят в Forge
  # только краем (Внутренняя Монголия, Чунцин, Гуанси, Сычуань), нарочно
  # не расписаны: своей линии они не дают, а срез сам прирастает к
  # соседней провинции при разрезании.
  "by_state": {
"China": {
    "Jiangsu": "nanjing", "Shanghai": "nanjing", "Anhui": "nanjing",

    "Zhejiang": "fuzhou", "Fujian": "fuzhou",

    "Guangdong": "guangzhou",

    "Hubei": "wuhan", "Hunan": "wuhan", "Jiangxi": "wuhan",

    # Шэньси заходит в Forge только восточной частью — западная
    # отрежется вместе с чужой землёй.
    "Shaanxi": "xian", "Shanxi": "xian", "Henan": "xian",

    "Shandong": "tianjin", "Hebei": "tianjin",
    "Beijing": "tianjin", "Tianjin": "tianjin",

    "Liaoning": "shenyang", "Jilin": "shenyang",
},
  },
}
...
# ═══════════════ ЧТЕНИЕ ИСХОДНИКОВ ═══════════════
FACTIONS["shumery"] = {
  "title": "Конфедерация Междуречья",
  "regions": [
    ("akkad",     "Аккад",     "Аккад",     33.315,  44.366),
    ("ur",        "Ур",        "Ур",        30.508,  47.783),
    ("uruk",      "Урук",      "Урук",      31.320,  45.640),
    ("lagash",    "Лагаш",     "Лагаш",     31.550,  46.950),
    ("shuruppak", "Шуруппак",  "Шуруппак",  35.560,  45.440),
    ("umma",      "Умма",      "Умма",      33.115,  45.950),
    ("nippur",    "Ниппур",    "Ниппур",    32.120,  45.230),
    ("babilu",    "Баб-Илу",   "Баб-Илу",   35.335,  40.145),
    ("eridu",     "Эриду",     "Эриду",     30.960,  47.000),
  ],
  # Сирия целиком уходит в западные ворота, Турция — в северный полис,
  # аравийская и кувейтская кромки — к ближайшим. Внутрь Конфедерации
  # от них попадают только приграничные полосы, остальное отрежется.
  "by_country": {
      "Syria": "babilu", "Jordan": "babilu",
      "Turkey": "shuruppak",
      "Saudi Arabia": "uruk",
      "Kuwait": "ur",
  },
  "by_state": {
    "Iraq": {
        # Аккад — Багдад и Тигр выше него
        "Baghdad": "akkad", "Sala ad-Din": "akkad",
        # Ур — Шатт-эль-Араб и выход к заливу
        "Al-Basrah": "ur",
        # Урук — нижний Евфрат
        "Al-Muthannia": "uruk",
        # Лагаш — орошаемая равнина Амара — Насирия
        "Maysan": "lagash", "Dhi-Qar": "lagash",
        # Шуруппак — Загрос и северная равнина
        "As-Sulaymaniyah": "shuruppak", "Arbil": "shuruppak",
        "Dihok": "shuruppak", "At-Ta'mim": "shuruppak",
        "Ninawa": "shuruppak",
        # Умма — восточный щит вдоль иранской границы
        "Wasit": "umma", "Diyala": "umma",
        # Ниппур — храмовое междуречье
        "Al-Q\u0101disiyyah": "nippur",
        "Babil": "nippur", "An-Najaf": "nippur", "Karbala'": "nippur",
        # Баб-Илу — западная пустыня и сирийский коридор
        "Al-Anbar": "babilu",
    },
    "Iran": {
        "Ilam": "umma", "Kermanshah": "umma",
        "Khuzestan": "ur",
        "*": "shuruppak",
    },
  },
  # Болота не совпадают ни с одной мухафазой: Эриду вырезается по месту.
  # Хаммарские и центральные болота от Чибайиша до кромки пустыни;
  # Басра и Насирия остаются за краем.
  "shapes": {
    "eridu": [
      (31.42, 46.45), (31.40, 46.95), (31.30, 47.35), (31.05, 47.60),
      (30.72, 47.62), (30.52, 47.30), (30.55, 46.85), (30.72, 46.45),
      (31.05, 46.28),
    ],
  },
}
...
FACTIONS["jamahiriya"] = {
  ...
  "regions": [
    ...
    ("nejd",  "Надждская группа",     "Эр-Рияд",  24.713, 46.675),
...
FACTIONS["australia"] = {
  "title": "Австралийский протекторат",
  "regions": [
    ("sydney",     "Сиднейская зона",     "Сидней",       -33.868, 151.209),
    ("melbourne",  "Мельбурнская зона",   "Мельбурн",     -37.814, 144.963),
    ("brisbane",   "Брисбенская зона",    "Брисбен",      -27.469, 153.025),
    ("perth",      "Пертская зона",       "Перт",         -31.953, 115.857),
    ("adelaide",   "Аделаидская зона",    "Аделаида",     -34.928, 138.600),
    ("canberra",   "Канберрская зона",    "Канберра",     -35.281, 149.128),
    ("townsville", "Таунсвиллская зона",  "Таунсвилл",    -19.258, 146.817),
    ("darwin",     "Дарвинская зона",     "Дарвин",       -12.462, 130.842),
    ("alice",      "Центральная зона",    "Алис-Спрингс", -23.698, 133.881),
    ("tasmania",   "Тасманская зона",     "Хобарт",       -42.881, 147.327),
  ],
  # Линии проведены по рубежам достижимости служб: кромкам пустынь,
  # водоразделам, трансконтинентальным дорогам и железным коридорам.
  # Каждая упирается либо в рамку, либо точно в вершину соседней линии —
  # общие вершины выписаны одинаковыми числами намеренно.
  "cuts": [
    # A — западный рубеж: Перт отделён от всего остального
    [(-8.0, 119.5), (-15.0, 119.0), (-19.6, 120.9), (-21.5, 123.0),
     (-23.0, 125.0), (-25.0, 126.3), (-27.5, 126.4), (-30.0, 126.0),
     (-32.2, 125.3), (-46.0, 124.0)],
    # B — северный рубеж: Дарвин отделён от Центральной
    [(-21.5, 123.0), (-20.2, 126.0), (-19.4, 130.0), (-18.6, 134.0),
     (-17.9, 137.3), (-17.3, 138.4)],
    # C — залив Карпентария: Дарвин отделён от Таунсвилла
    [(-17.3, 138.4), (-14.0, 138.0), (-10.5, 137.6), (-8.0, 137.4)],
    # D — северо-восточный рубеж: Таунсвилл отделён от внутренних зон
    [(-17.3, 138.4), (-19.2, 140.8), (-20.8, 143.0), (-21.9, 145.5),
     (-22.6, 148.0), (-23.0, 151.0), (-23.3, 156.0)],
    # E — восточный рубеж Центральной: отделена от Брисбена
    [(-21.9, 145.5), (-24.5, 146.0), (-27.0, 146.4), (-29.3, 146.6)],
    # H — южный рубеж Центральной
    [(-30.0, 126.0), (-30.2, 130.0), (-30.4, 134.0), (-30.6, 138.0),
     (-30.8, 141.5), (-30.0, 144.0), (-29.3, 146.6)],
    # F — Брисбен отделён от Сиднея
    [(-29.0, 156.0), (-28.9, 153.6), (-29.2, 151.5), (-29.6, 149.0),
     (-29.3, 146.6)],
    # I — Аделаида отделена от Сиднея
    [(-29.3, 146.6), (-31.0, 145.6), (-32.6, 144.8), (-33.8, 144.0)],
    # J — Аделаида отделена от Мельбурна
    [(-33.8, 144.0), (-35.2, 142.8), (-36.8, 141.6), (-38.5, 140.8),
     (-46.0, 140.2)],
    # K — Мельбурн отделён от Канберры и Сиднея
    [(-33.8, 144.0), (-35.0, 145.8), (-36.0, 147.5), (-36.8, 149.3),
     (-37.3, 150.5), (-37.6, 156.0)],
    # L — карман Канберры внутри юго-востока
    [(-36.0, 147.5), (-35.0, 147.8), (-34.3, 148.8), (-34.2, 150.2),
     (-35.0, 150.9), (-36.8, 149.3)],
    # M — Бассов пролив: Тасмания отделена от материка
    [(-39.6, 110.0), (-39.4, 143.0), (-39.8, 148.5), (-39.9, 156.0)],
  ],
}


def build_from_cuts(spec, area):
    """Образцы регионов из нарисованных линий разреза.

    Границы зон ответственности Австралийского протектората не наследуют
    ничьих прежних линий: их провели по водоразделам, дорогам, железным
    коридорам и рубежам достижимости аварийных служб. Задавать их
    замкнутыми полигонами было бы мучением — соседние кольца пришлось бы
    сводить вершина в вершину. Поэтому задаются только сами линии, а
    куски из них собирает shapely: рамка вокруг фракции плюс все разрезы
    режутся на грани, и каждая грань достаётся тому региону, чья столица
    внутри неё лежит.

    Линия должна упираться обоими концами либо в рамку, либо точно в
    вершину другой линии — иначе она ничего не разрежет. Висячий хвост
    не ошибка: shapely его просто не возьмёт в грани, так что
    перехлёстывать концы безопасно.
    """
    from shapely.geometry import LineString, Point, box
    from shapely.ops import unary_union, polygonize
    x0, y0, x1, y1 = area.bounds
    m = 2.0
    frame = box(x0 - m, y0 - m, x1 + m, y1 + m)
    lines = [frame.boundary]
    for cut in spec["cuts"]:
        lines.append(LineString([(lo, la) for la, lo in cut]))
    faces = list(polygonize(unary_union(lines)))

    seeds = {k: Point(lo, la) for k, _n, _c, la, lo in spec["regions"]}
    tpl, dry = {}, 0
    for f in faces:
        hit = [k for k, p in seeds.items() if f.contains(p)]
        if len(hit) > 1:
            sys.exit(f"  в один кусок попали столицы: {', '.join(hit)} — "
                     f"не хватает линии между ними")
        if not hit:
            # Кусок без столицы — это почти всегда море за краем
            # материка. А вот если в нём оказалась суша, значит линия
            # разрезала чью-то зону надвое, и это уже ошибка.
            if f.intersects(area) and area_km2(f.intersection(area)) > 500:
                c = f.centroid
                sys.exit(f"  кусок суши {area_km2(f.intersection(area)):.0f} км² "
                         f"около {c.y:.1f}, {c.x:.1f} не достался никому")
            dry += 1
            continue
        tpl.setdefault(hit[0], []).append(f)
    print(f"  разрезов {len(spec['cuts'])}, кусков {len(faces)}, "
          f"из них пустых (море) {dry}")
    return {k: unary_union(v) for k, v in tpl.items()}
...
def build_templates(feats, spec):
    ...
    from shapely.ops import unary_union
    from shapely import make_valid
    by_country = spec.get("by_country", {})
    by_state = spec.get("by_state", {})
    ...
    tpl = {k: unary_union(v) for k, v in groups.items() if v}
    return apply_shapes(tpl, spec)


def apply_shapes(tpl, spec):
    """Регион, заданный географией, а не бывшей границей.

    Очертания вырезаются из всех образцов, которых касаются, и
    становятся собственным образцом. Дальше разрезание идёт как обычно:
    новая линия возьмётся с края этих очертаний, как берётся с края
    мухафазы или земли.
    """
    from shapely.geometry import Polygon
    from shapely.ops import unary_union
    ...
    for key, outline in shapes.items():
        poly = make_valid(Polygon([(lo, la) for la, lo in outline]))
        for k in list(tpl):
            if k == key or not tpl[k].intersects(poly):
                continue
            tpl[k] = make_valid(tpl[k].difference(poly))
            if tpl[k].is_empty:
                del tpl[k]
        tpl[key] = make_valid(unary_union([tpl[key], poly]) if key in tpl else poly)
    return tpl
...
def build_faction(fkey, spec, feats):
    ...
    templates = (build_from_cuts(spec, area) if spec.get("cuts")
                 else build_templates(feats, spec))
